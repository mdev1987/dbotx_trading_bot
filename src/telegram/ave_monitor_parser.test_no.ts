// Tests for the AVE Scanner signal parser
...
// Sample raw BULLHOUSE signal message from Telegram
...
// Sample raw CLICK signal message
const clickMessage = `
💠 New Solana Pool Launched 💠
...
// Test parsing BULLHOUSE signal — validates MCap, liquidity, pair, holders
test("parse BULLHOUSE signal", () => {
  const result = parseAveScannerSignal(bullhouseMessage);
  // console.dir(result);
...
// Test parsing CLICK signal with different MCap and pair format
test("parse CLICK signal", () => {
  const result = parseAveScannerSignal(clickMessage);
  // console.dir(result);
...
// Tests for the expandCompressedDecimal helper function
...
// Tests for the parseAbbreviatedUsd helper function
...
// Markdown-formatted signal message (as received from Telegram)
...
// Expected result after removing markdown from signal_message
...
// Test that remove-markdown correctly strips markdown from Telegram messages
test("Parse Markdown", () => {
...
// CJ signal with expanded decimal notation
const cjMessage = `
💠 New Solana Pool Launched 💠
...
// Belmar coin signal — token name with space, normal decimal price
const belmarMessage = `
💠 New Solana Pool Launched 💠
...
// Catchua signal with compressed decimal price
...
// OTKEN signal with MCap $0.00 edge case
const otkenMessage = `
💠 New Solana Pool Launched 💠
...
// Bwull signal — Security Score 55 (🟢Low Risk)
const bwullMessage = `
💠 New Solana Pool Launched 💠
...
// ANSEM signal — K pair amount, Meteoradammv2 DEX
const ansemMessage = `
💠 New Solana Pool Launched 💠
...
// Woman signal — 2 holders, Pump.fun link in external links
const womanMessage = `
💠 New Solana Pool Launched 💠
...
// DATA signal with compressed decimal price
const dataMessage = `
💠 New Solana Pool Launched 💠
...
// Dream signal — multiple compressed decimals in holders
const dreamMessage = `
💠 New Solana Pool Launched 💠
...
// SPAM signal — 14 snipes, Security Score 55
const spamMessage = `
💠 New Solana Pool Launched 💠
...
// Test CJ signal parsing — validates all fields including expanded decimal
test("parse CJ signal", () => {
  const r = parseAveScannerSignal(cjMessage);
  expect(r.tokenName).toBe("CJ");
  expect(r.contractAddress).toBe(
    "7XsrVVXftKDpbocq5SX3wQ9QhwiPcqinJ5L1dNHyJvHB",
  );
...
// Test Belmar coin — token name with space, high MCap
test("parse Belmar coin signal", () => {
  const r = parseAveScannerSignal(belmarMessage);
...
// Test Catchua signal — compressed decimal init price
test("parse Catchua signal", () => {
  const r = parseAveScannerSignal(catchuaMessage);
...
// Test OTKEN — handles MCap of $0.00 correctly
test("parse OTKEN signal (MCap $0.00)", () => {
  const r = parseAveScannerSignal(otkenMessage);
...
// Test Bwull — Security Score 55, compressed decimal
test("parse Bwull signal (Score 55)", () => {
  const r = parseAveScannerSignal(bwullMessage);
...
// Test ANSEM — K pair, Meteoradammv2 DEX, 101 insiders
test("parse ANSEM signal (K pair, Meteoradammv2)", () => {
  const r = parseAveScannerSignal(ansemMessage);
...
// Test Woman — 2 holders, Pump.fun link in external links
test("parse Woman signal (2 holders, Pump.fun link)", () => {
  const r = parseAveScannerSignal(womanMessage);
...
// Test DATA signal — compressed decimal parsing
test("parse DATA signal", () => {
  const r = parseAveScannerSignal(dataMessage);
...
// Test Dream — 0.0{5}6883 price + 0.0{4}7038 holder percentage
test("parse Dream signal (0.0{5}6883 + 0.0{4}7038 holder%)", () => {
  const r = parseAveScannerSignal(dreamMessage);
...
// Test SPAM — Score 55, 14 snipes, 8 insiders
test("parse SPAM signal (Score 55, 14 snipes)", () => {
  const r = parseAveScannerSignal(spamMessage);
...
// USA250 signal — missing Token Holders line entirely
const usa250Message = `
💠 New Solana Pool Launched 💠
...
// Test USA250 — validates parsing without Token Holders line
test("parse USA250 signal (no Token Holders line)", () => {
  const r = parseAveScannerSignal(usa250Message);
  expect(r.tokenName).toBe("USA250");
  expect(r.contractAddress).toBe(
    "D49tCt3GB8Q8HYYK35ojTq3f6y1XaS6FgjJ8j3zNDwf1",
  );
