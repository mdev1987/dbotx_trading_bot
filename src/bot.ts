...
import { filter, map, withLatestFrom, tap, concatMap, share, shareReplay, scan, startWith } from "rxjs/operators";

import { CONFIG } from "./config";
import type { PositionState, PositionEvent, CloseReason, PriceUpdate, ParsedSignal, PerformanceReport } from "./types";
import {
  priceUpdate$, subscribePairs, unsubscribePair, pushPriceUpdate,
  simBuy, simSell, liveBuy, liveSell, pollOrderUntilDone, querySwapOrder,
  fetchTradePairs, fetchPnLTasks, fetchBuyTrade, startRestPricePolling, startSimPairPoll, startSimPnLPoll,
  tradeResultEvent$, buySuccessEvent$, sellSuccessEvent$, tpSuccessEvent$, slSuccessEvent$,
  refreshAccount$, latestSimAccount, simAccount$,
} from "./dbotx";
import { acceptedSignal$, signalPaused$, expiredPair$, wireReporter } from "./telegram";
...
type Command = { type: "upsert"; id: number; pos: PositionState } | { type: "patch"; id: number; patch: Partial<PositionState> } | { type: "remove"; id: number };
...
export const positions$: Observable<Map<number, PositionState>> = command$.pipe(
  scan((map, cmd) => {
    switch (cmd.type) {
      case "upsert": { const m = new Map(map); m.set(cmd.id, cmd.pos); return m; }
      case "patch": {
      ...
      case "remove": { const m = new Map(map); m.delete(cmd.id); return m; }
      default: return map;
    }
...
positions$.subscribe((m) => { _latestPositions.clear(); for (const [k, v] of m) _latestPositions.set(k, v); });

export const openPositions$: Observable<PositionState[]> = positions$.pipe(
  map((m) => { const open: PositionState[] = []; for (const p of m.values()) { if (p.status === "open" || p.status === "closing") open.push(p); } return open; }),
  startWith([] as PositionState[]),
  shareReplay({ bufferSize: 1, refCount: true }),
);

export const openOrderIds$: Observable<string[]> = openPositions$.pipe(
  map((ps) => ps.filter((p) => p.orderId && !p.orderId.startsWith("paper_")).map((p) => p.orderId)),
);

const positionEventInput$ = new Subject<PositionEvent>();
export const positionEvent$: Observable<PositionEvent> = positionEventInput$.pipe(share());
export const positionClosed$: Observable<PositionEvent> = positionEvent$.pipe(filter((e) => e.type === "closed"));

function emitEvent(event: PositionEvent): void {
  console.log(`[BOT] ${event.position.tokenName}: ${event.type}${event.detail ? ` - ${event.detail}` : ""}`);
...
async function captureEntryPriceSim(orderId: string, positionId: number): Promise<void> {
  for (let i = 0; i < CONFIG.maxEntryPriceAttempts; i++) {
    try {
      const trade = await fetchBuyTrade(orderId);
      if (trade && trade.priceUsd > 0) {
        patchPosition(positionId, { entryPriceUsd: trade.priceUsd, peakPriceUsd: trade.priceUsd, entryCostUsd: trade.totalUsd });
        return;
      }
    } catch { /* retry */ }
...
async function captureEntryPriceLive(orderId: string, positionId: number): Promise<void> {
  try {
    const order = await pollOrderUntilDone(orderId);
    if (order.txPriceUsd && order.txPriceUsd > 0) {
      patchPosition(positionId, { entryPriceUsd: order.txPriceUsd, peakPriceUsd: order.txPriceUsd });
    }
  } catch (err) {
...
export async function openPosition(signal: ParsedSignal): Promise<number> {
  ...
  for (const pos of _latestPositions.values()) {
    if (pos.pair === signal.lpAddress && (pos.status === "open" || pos.status === "closing")) return 0;
  }
  if (_pendingBuys.has(signal.lpAddress)) return 0;

  _pendingBuys.add(signal.lpAddress);
  setTimeout(() => _pendingBuys.delete(signal.lpAddress), CONFIG.pendingBuyTtlMs);
  ...
  try {
    ...
    const position: PositionState = {
      id: positionId, orderId, pair: signal.lpAddress, token: signal.contractAddress,
      tokenName: signal.tokenName ?? "unknown", tokenSymbol: "",
      entryPriceUsd: null, entryCostUsd: null, sizeSol,
      filledSol: CONFIG.liveMode ? 0 : sizeSol, avgFillPriceUsd: null,
      peakPriceUsd: 0, trailingActive: false,
      currentProfitPercent: 0, currentProfitUsd: 0, remainingBalance: "0",
      openedAt: now, expiresAt: now + CONFIG.baseTtlSecs * 1000, lastUpdateAt: now,
      status: "open", closeReason: null, exitPriceUsd: null, signal,
    };

    upsertPosition(position);
    subscribePairs([{ pair: signal.lpAddress, token: signal.contractAddress }]);
    emitEvent({ type: "opened", position, detail: `${signal.tokenName} @ ${sizeSol.toFixed(4)} SOL` });
...
export async function closePositionById(id: number, reason: CloseReason, detailOverride?: string): Promise<void> {
  ...
  const final: PositionState = { ...latest, status: "closed", closeReason: reason, lastUpdateAt: Date.now() };
...
async function fetchFinalPnL(tokenAddress: string): Promise<{ profitPct: number; profitUsd: number; exitPriceUsd: number | null } | null> {
  for (let i = 0; i < 10; i++) {
    try {
...
export async function closePosition(pair: string, reason: CloseReason): Promise<void> {
  ...
  for (const [pid, p] of _latestPositions) {
    if (p.pair === pair && (p.status === "open" || p.status === "closing") && p.openedAt < oldestTime) {
...
acceptedSignal$.pipe(
  ...
  concatMap(async ([signal, open]) => {
    if (open.length >= CONFIG.maxPositions) {
      console.log(`[BOT] Max positions (${CONFIG.maxPositions}) reached — skipping ${signal.tokenName}`);
      return;
    }
...
priceUpdate$.pipe(
  ...
  tap(([update, positions]) => {
    for (const pos of positions) {
      if (pos.pair !== update.pair || !update.priceUsd || update.priceUsd <= 0) continue;
      if (!pos.entryPriceUsd) {
        patchPosition(pos.id, { entryPriceUsd: update.priceUsd, peakPriceUsd: update.priceUsd });
      }
      if (update.priceUsd > pos.peakPriceUsd) {
        pos.peakPriceUsd = update.priceUsd;
      }
      const profitPct = (update.priceUsd - pos.entryPriceUsd!) / pos.entryPriceUsd!;
      const profitUsd = profitPct * pos.sizeSol;
      patchPosition(pos.id, { currentProfitPercent: profitPct, currentProfitUsd: profitUsd });
    }
  }),
).subscribe();
...
if (!CONFIG.liveMode) {
  priceUpdate$.pipe(
    withLatestFrom(openPositions$),
    filter(() => CONFIG.trailingDistancePct > 0 || CONFIG.trailingTpDistancePct > 0),
    tap(([update, positions]) => {
      for (const pos of positions) {
        if (pos.pair !== update.pair || !pos.entryPriceUsd) continue;
        const pnlFraction = (update.priceUsd - pos.entryPriceUsd) / pos.entryPriceUsd;
        const prevPeak = _pnlPeakMap.get(pos.id) ?? pnlFraction;
        if (pnlFraction > prevPeak) _pnlPeakMap.set(pos.id, pnlFraction);

        // Trailing TP
        if (CONFIG.trailingTpDistancePct > 0 && prevPeak > 0 && pnlFraction < prevPeak - CONFIG.trailingTpDistancePct) {
          closePositionById(pos.id, "take_profit").catch(() => {});
        }
        // Trailing stop
        if (CONFIG.trailingDistancePct > 0 && prevPeak > CONFIG.trailingActivationPct && pnlFraction < prevPeak - CONFIG.trailingDistancePct) {
          closePositionById(pos.id, "trailing_stop").catch(() => {});
        }
        // Hard stop loss
        if (CONFIG.stopLossPct < 0 && pnlFraction < CONFIG.stopLossPct && pnlFraction > -0.999) {
...
if (CONFIG.liveMode) {
  buySuccessEvent$.subscribe((event: any) => {
    ...
    if (pos && !pos.entryPriceUsd && event.result.priceUsd) {
      patchPosition(pos.id, { entryPriceUsd: event.result.priceUsd, peakPriceUsd: event.result.priceUsd });
  ...
  const closeFromWs = (event: any, reason: CloseReason) => {
    ...
    if (pos) {
      _pendingBuys.delete(pos.token);
      const final = { ...pos, status: "closed" as const, closeReason: reason, exitPriceUsd: event.result.priceUsd ?? null, lastUpdateAt: Date.now() };
      upsertPosition(final);
      unsubscribePair(pos.pair);
      emitEvent({ type: "closed", position: final, closeReason: reason, detail: `WS: ${reason}` });
      refreshAccount$.next();
...
timer(CONFIG.expiryCheckMs, CONFIG.expiryCheckMs).pipe(
  withLatestFrom(openPositions$),
  tap(([, positions]) => {
    const now = Date.now();
    ...
    for (const pos of positions) {
      ...
      if (CONFIG.minProfitForTtlExtensionPct > 0 && pos.currentProfitPercent >= CONFIG.minProfitForTtlExtensionPct) {
...
startSimPnLPoll(openOrderIds$, (orderId, tasks) => {
  ...
  const hasStopLoss = tasks.some((t) => t.state === "done" && t.triggerDirection === "down");
  const hasTakeProfit = tasks.some((t) => t.state === "done" && t.triggerDirection === "up");
  const anyTerminal = tasks.some((t) => t.state === "done" || t.state === "fail" || t.state === "expired");
...
startSimPairPoll(openPositions$, (pair: any) => {
  ...
  for (const pos of _latestPositions.values()) {
    ...
    patchPosition(pos.id, { remainingBalance: pair.token0Balance, currentProfitPercent: pair.fullProfitPercent * 100, currentProfitUsd: pair.fullProfitUsd });
    const balanceNum = Number(pair.token0Balance);
    if (balanceNum <= 0) {
      const reason: CloseReason = pair.sellProfitPercent !== null && pair.sellProfitPercent < 0 ? "stop_loss" : "take_profit";
      closePositionById(pos.id, reason).catch(() => {});
    }
  }
});
...
positionClosed$.pipe(
  filter((ev: any) => CONFIG.liveMode && !CONFIG.liveBuyEnabled && ev.position?.orderId?.startsWith("paper_")),
).subscribe((ev: any) => {
  const p = ev.position;
  const pnlSol = p.currentProfitUsd ?? 0;
  const cost = p.sizeSol;
  const exitPrice = p.exitPriceUsd ?? p.entryPriceUsd;
  const pnlPct = exitPrice && p.entryPriceUsd ? ((exitPrice - p.entryPriceUsd) / p.entryPriceUsd) : p.currentProfitPercent;
  const pnl = pnlPct * cost;
  _paperBalanceSol += cost + pnl;
  _paperRealizedPnLSol += pnl;
  if (pnl >= 0) _paperWins++; else _paperLosses++;
});
...
export function getReport(): PerformanceReport {
  ...
  for (const pos of _latestPositions.values()) {
    if (pos.status === "closed") {
      ...
      if (avgCount === 0 || pos.currentProfitPercent > bestPct) bestPct = pos.currentProfitPercent;
      if (avgCount === 0 || pos.currentProfitPercent < worstPct) worstPct = pos.currentProfitPercent;
      totalPct += pos.currentProfitPercent;
      avgCount++;
      if (pos.closeReason) reasons[pos.closeReason] = (reasons[pos.closeReason] ?? 0) + 1;
    }
  ...
  return {
    totalPositions: total, closedPositions: closedCount, openPositions: openCount,
    winningTrades: wins, losingTrades: losses,
    winRate: closedCount > 0 ? (wins / closedCount) * 100 : 0,
    totalProfitUsd, totalProfitPct: totalCostUsd > 0 ? (totalProfitUsd / totalCostUsd) * 100 : 0,
    avgProfitPct: avgCount > 0 ? totalPct / avgCount : 0,
    avgProfitUsd: closedCount > 0 ? totalProfitUsd / closedCount : 0,
    bestTradePct: bestPct, worstTradePct: worstPct, reasons,
  };
}
...
export function getPaperBalanceSol(): number { return _paperBalanceSol; }
export function getPaperRealizedPnLSol(): number { return _paperRealizedPnLSol; }
...
export async function recoverOpenPositions(): Promise<void> {
  ...
  try {
    ...
    for (const p of pairs) {
      const entryPriceUsd = p.costUsd > 0 && Number(p.buyTokenAmount) > 0 ? p.costUsd / Number(p.buyTokenAmount) : null;
      const pos: PositionState = {
        id: _nextId++, orderId: p._id, pair: p._id,
        token: p.tokenInfo0.contract, tokenName: p.tokenInfo0.name ?? p.tokenInfo0.symbol ?? "unknown", tokenSymbol: p.tokenInfo0.symbol ?? "",
        entryPriceUsd, entryCostUsd: p.costUsd, sizeSol: Number(p.buyTokenAmount) || 0, filledSol: 0, avgFillPriceUsd: null,
        peakPriceUsd: 0, trailingActive: false, currentProfitPercent: (p.fullProfitPercent ?? 0) * 100, currentProfitUsd: p.fullProfitUsd ?? 0,
        remainingBalance: String(Number(p.buyTokenAmount) || 0), openedAt: now, expiresAt: now + CONFIG.baseTtlSecs * 1000,
        lastUpdateAt: now, status: "open", closeReason: null, exitPriceUsd: null,
        signal: { tokenName: p.tokenInfo0.name ?? p.tokenInfo0.symbol ?? "unknown", contractAddress: p.tokenInfo0.contract, lpAddress: p._id, chain: "solana" },
      };
      upsertPosition(pos);
    }
    if (pairs.length > 0) {
      subscribePairs(pairs.map((p) => ({ pair: p._id, token: p.tokenInfo0.contract })));
    }
  } catch (err) {
